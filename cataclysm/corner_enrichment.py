...
ELEVATION_FLAT_THRESHOLD_PCT = 1.0
ELEVATION_GRADE_THRESHOLD_PCT = 1.5
ELEVATION_CREST_GRADE_DELTA_PCT = 2.0
ELEVATION_APEX_WINDOW_M = 20.0
...
def auto_enrich_corner_metadata(
    ...
    for corner in corners:
        _auto_detect_elevation_trend(corner, lap_data)
    ...
    for corner in corners:
        _auto_detect_blind_crest(corner, lap_data)
...
def _auto_detect_character(
    ...
    window_start = corner.apex_distance_m - CHAR_APPROACH_BEFORE_APEX_M
    window_end = corner.apex_distance_m - CHAR_APPROACH_END_BEFORE_M
    window_start = max(window_start, corner.entry_distance_m)
    ...
    if max_brake >= CHAR_BRAKE_PCT_THRESHOLD and has_sustained_braking:
    ...
    if (
        ...
        and min_throttle > CHAR_MAX_THROTTLE_PCT
    ...
    if max_brake >= CHAR_BRAKE_PCT_THRESHOLD or (speed_drop_pct > 8.0 and has_decel):
        corner.character = "brake"
...
def _auto_detect_elevation_trend(
    ...
    entry_alt = float(altitude[entry_idx])
    exit_alt = float(altitude[exit_idx])
    ...
    trend = _classify_elevation_trend(
        distance, altitude, apex_idx, entry_idx, exit_idx, gradient_pct
    )
...
def _auto_detect_camber(
    ...
    for lap in all_lap_data.values():
        ...
        zone_start = corner.entry_distance_m + 0.25 * corner_len
        zone_end = corner.exit_distance_m - 0.25 * corner_len
        ...
        steady_mask = kin_diff < np.percentile(kin_diff, 70)

        valid = (
            (speed_seg >= CAMBER_MIN_SPEED_MPS)
            & (brake_seg <= CAMBER_MAX_BRAKE_PCT)
            & np.isfinite(kin_seg)
            & np.isfinite(lat_g_seg)
            & steady_mask
        )
        if not np.any(valid):
            continue
...
def _auto_detect_blind_crest(
    ...
    crest_blind = _check_crest_blindness(distance, altitude, turnin_idx, apex_idx)
...
def _check_crest_blindness(
    ...
    for idx in range(turnin_idx + 1, apex_idx):
        ...
        if float(smooth_alt[idx] - los_z) > BLIND_CREST_THRESHOLD_M:
            return True
    return False
...
def _smooth_altitude(altitude: np.ndarray, distance: np.ndarray, window_m: float) -> np.ndarray:
    ...
    window_pts = max(3, int(window_m / mean_step))
