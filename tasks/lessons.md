...
## NPZ File Slug Must Match track_slug_from_layout() Output (2026-03-20)

**Pattern**: When creating track reference NPZ files, the filename slug MUST match what `track_slug_from_layout(layout)` generates from the TrackLayout `name` field. The slug is generated by lowercasing, stripping non-alphanumeric, and joining with hyphens. E.g., "WeatherTech Raceway Laguna Seca" → `weathertech-raceway-laguna-seca`, NOT `laguna-seca`.

**Why**: OSM build script used short slugs (`laguna-seca.npz`, `road-atlanta.npz`) but the editor API loads NPZ by the auto-generated slug from the track name. Result: 404 in track editor for all new tracks. Required rebuilding all NPZ files with correct slugs.

**Error signature**: Track appears in the editor dropdown but selecting it returns 404. Check: does the NPZ filename match `track_slug_from_layout(layout)`?

## Proxy-Generation Vehicle DB Entries Pollute Validation Data (2026-03-20)

**Pattern**: NEVER add validation entries where the car_key maps to a different generation than the actual car. E.g., BMW M4 F83 (convertible) mapped to F82 (coupe) or Audi TT RS 8J mapped to 8S. Different generations have different weight, power, and aero — the solver error reflects the spec mismatch, not physics accuracy.

**Why**: Two proxy entries (M4 F83 ratio=0.905, TT RS 8J ratio=0.861) appeared as massive IQR outliers, distorting mean and std. Removing them immediately improved dataset quality. Only add entries where vehicle_db has the EXACT generation.

**Error signature**: A validation entry with ratio <0.90 or >1.10 where the car_key generation doesn't match the actual car's generation.

## Professional Driver Data Needs Separate Acceptance Gates (2026-03-20)

**Pattern**: When integrating professional driver lap times (C&D Lightning Lap, MotorTrend), split acceptance criteria by `driver_level`. Pros achieve 95-98% of physics limit → ratio ~1.15-1.20 is expected. Mixing pro data into amateur-calibrated gates (mean ~1.00, exceedance threshold 8) causes immediate failure. Gate amateur data only; report pro data as diagnostics.

**Why**: Adding 82 pro entries caused 88 exceedances >1.05 (threshold: 8) and mean shifted to 1.10. The solver isn't wrong — the acceptance criteria were calibrated for amateurs. Fix: filter `results` to `amateur_results` (driver_level != "pro") for acceptance gates, report pro breakdown separately.

**Error signature**: All acceptance checks fail after adding pro driver data. Mean ratio shifts to ~1.15-1.20. Fix is always to split by driver_level, not to adjust thresholds.
